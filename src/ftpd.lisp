(uiop/package:define-package :aftpd/src/ftpd (:nicknames)
                             (:use :aftpd/src/config :aftpd/src/ipaddr :cl) (:shadow) (:export)
                             (:intern))
(in-package :aftpd/src/ftpd)
;;don't edit above
...
(eval-when (compile load eval)
  (defparameter *extfcrlf*
    (excl:find-composed-external-format :e-crlf (excl:crlf-base-ef :latin1))))
    ...
    (while (not (= 0 (setf got (read-sequence inbuffer f :partial-fill t))))
	   (multiple-value-bind (ignore count)
	       (string-to-octets inbuffer 
				 ...
				 :mb-vector outbuffer
				 :end got
				 #+allegro :external-format #+allegro *extfcrlf*)
...
(defun main (&key debug-mode configfile ftpport test-mode)
  (when ftpport
